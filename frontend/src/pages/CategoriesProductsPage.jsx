...
const CategoriesProductsPage = () => {
	...
	useEffect(() => {
		axios
			.get(`${import.meta.env.VITE_API_BASE_URL}/categories/${id}/products`)
			.then((res) => {
				...
				<div className='flex flex-col gap-2'>
					...
					{loader ? (
						...
						<motion.div
							...
							initial='hidden'
							animate='visible'
							className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-10 justify-items-center lg:justify-items-start'
						>
								...
								return (
									<motion.div variants={itemVariants} key={product.product_id}>
										<ProductCard
											...
											image_url={
												product.image_url ||
												'https://cdn.greatnews.life/wp-content/uploads/2022/11/Untitled-design-21.png'
											}
											name={product.name}
											description={
							...
							<motion.div
								...
								className=' w-[90vw] h-full sm:w-[45vw] lg:w-[30vw] xl:max-w-[400px] rounded-3xl bg-white shadow-lg overflow-hidden transition-transform duration-300 hover:-translate-y-1 border-3 border-dotted border-white hover:border-blue-500 group cursor-pointer'
							>
								<div className=' flex h-80 flex-col gap-4 justify-center items-center'>
									...
									<p className='text-center text-[#62748E]'>
										Add a new product in this category
									</p>
								</div>
							</motion.div>
						</motion.div>
