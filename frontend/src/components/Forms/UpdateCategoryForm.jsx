	...
	return (
		<form onSubmit={handleSubmit}>
			...
			{preview ? (
				<div className='relative w-20 h-20 mb-4'>
					<img
						src={preview}
						alt='Preview'
						className='w-full h-full object-cover rounded-xl border-2 border-indigo-500'
					/>
					<button
						type='button'
						onClick={() => {
			...
			<button
				type='submit'
				disabled={loading}
				className='relative w-full mt-6 h-12 rounded-xl bg-gradient-to-r from-indigo-600 to-emerald-500 text-white font-bold hover:text-lg hover:font-extrabold transition-all cursor-pointer'
			>
				{loading ? (
					<div className='absolute loaderTwo text-white top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%]'></div>
				) : (
					'Update Category'
				)}
			</button>
