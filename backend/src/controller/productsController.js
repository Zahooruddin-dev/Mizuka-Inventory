...
async function updateProduct(req, res) {
	...
	try {
		...
		const product = await db.updateProductById(
			id,
			name,
			price,
			description,
...
async function fetchProductsForCategory(req, res) {
...
async function createNewProduct(req, res) {
	...
	try {
		const { name, price, description, category_id, image_url, stock } =
			req.body;
		const product = await db.createNewProduct(
			name,
			price,
			description,
			category_id,
