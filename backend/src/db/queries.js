...
async function getAllProducts() {
	...
	const { rows } = await pool.query(
		'SELECT p.product_id,name,price,description,category_id,image_url,quantity FROM public.products p JOIN public.inventory i ON p.product_id = i.product_id',
	);
	return rows;
}
...
async function deleteProductById(id) {
	...
	const { rows } = await pool.query(
		'DELETE FROM public.products where product_id = $1 RETURNING *',
		[id],
	);
	return rows[0];
}
...
async function getProductsByCategory(id) {
	...
	const { rows } = await pool.query(
		'SELECT p.*,i.quantity FROM public.products p JOIN public.inventory i ON p.product_id = i.product_id WHERE category_id = $1',
		[id],
	);
	return rows;
}
